...
public class MetaDataTest {
    ...
    public MetaDataTest(String methodCall) throws SQLException {
        ...
        try {
            Method m = DatabaseMetaData.class.getMethod(m_methodName,
                                                        m_methodArgTypes);
            ...
            while (rs.next()) {
                ...
                for (int i = 1; i <= cnt; i++) {
                    ...
                    result.append(rsObject.toString().replaceAll("(\\\\|;)",
                                                                 "\\$1")
                                  + ";");
                }
                m_results.add(result.toString());
            ...
            for (Class<?> m_methodArgType : m_methodArgTypes) {
                if (sb.length() > 0) {
                    sb.append(",");
                }
                sb.append(m_methodArgType.getName());
            }
            throw new SQLException(
                                   "No such method or non-resultset return type: "
                                           + m_methodName + "(" + sb.toString()
                                           + ")");
    ...
    /**
     * Method call parser. Let's say that only String, String[], int, int[] and
     * boolean types are accepted. Examples: "foo" => String {"foo","bar"} =>
     * String[] {} => String[] (zero length array, not null!) 10 => int -7 =>
     * int [10,3] => int[] [] => int[] (zero length array, not null!) TRUE =>
     * boolean (String)null => String (value null) (String[])null => String[]
     * (value null) (int[])null => int[] (value null)
     * 
     * Complete example: select * from
     * callMetaDataMethod('getTables((String)null,"sqlj","jar%",{"TABLE"})');
     * 
    ...
    private void parseMethodCall(String methodCall) throws SQLException {
        try {
            ...
            if (m.matches()) {
                ...
                p = Pattern.compile("^\\s*("
                                    + "\\(\\s*(?:String|int)\\s*(?:\\[\\s*\\])?\\s*\\)\\s*null|"
                                    + //String, String[] or int[] null
                                    "TRUE|"
                                    + //boolean TRUE
                                    "FALSE|"
                                    + //boolean FALSE
                ...
                while (!auxParamString.equals("")) {
                    ...
                    if (!m.matches()) {
                        throw new SQLException("Invalid parameter list: "
                                               + paramString);
                    }
                    ...
                    } else if (param.startsWith("{")) //it is a string array
                    {
                        ...
                        while (!auxParamArr.equals("")) {
                            ...
                            if (!marr.matches()) {
                                throw new SQLException("Invalid string array: "
                                                       + param);
                            }
                    ...
                    } else if (param.startsWith("(")) //it is String, String[] or int[] null
                    {
                        ...
                        if (mnull.matches()) {
                            ...
                            throw new SQLException("Invalid null value: "
                                                   + param);
                        }
                    } else if (param.startsWith("[")) //it is a int array
                    {
                        ...
                        while (!auxParamArr.equals("")) {
                            marr = parr.matcher(auxParamArr);
                            if (!marr.matches()) {
                                throw new SQLException("Invalid int array: "
                                                       + param);
                            }
            ...
            m_methodArgTypes = types.toArray(new Class[0]);
        } catch (Exception e) {
            throw new SQLException("Invalid method call: " + methodCall
                                   + ". Cause: " + e.toString());
        }
    }
}
